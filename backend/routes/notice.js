...
router.post("/", async (req, res) => {
  ...
  try {
    const { businessGroup, purpose, title, content, isPinned, authorId } = req.body;

    // 새 문의글 생성
    await Notice.create(db, {
      businessGroup,
      purpose,
      title,
      content,
    ...
    res.status(201).json({ message: "공지사항이 성공적으로 등록되었습니다." });
  } catch (error) {
...
router.get("/", async (req, res) => {
  ...
  try {
    const notice = await Notice.findNoticeAll(db);
...
router.get("/:id", async (req, res) => {
  ...
  try {
    const notice = await Notice.findReqById(db, req.params.id);
...
router.put("/:id", async (req, res) => {
  ...
  try {
    ...
    const result = await Notice.update(db, {
      id: req.params.id,
      businessGroup,
      purpose,
      title,
