...
const AdminContacts = () => {
  ...
  const handleStatusUpdate = async (newStatus) => {
    try {
      await axios.put(
        `http://localhost:3001/api/contact/${selectedContact.id}`,
        { status: newStatus },
      ...
      Swal.fire("오류 벌샹", "수정 중 문제가 발생했습니다.", "error");
    }
  };
  ...
  const handleDelete = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        Swal.fire("삭제완료!", "문의가 성공적으로 수정되었습니다.", "success");
      } catch (error) {
  ...
  const filteredContacts = useMemo(() => {
    return contacts.filter((contact) => {
      const fieldValue = contact[searchType];
      const value = (fieldValue || "").toLowerCase();
      const matchesSearch = value.includes(searchTerm.toLowerCase());
      const matchesStatus =
        statusFilter === "" || String(contact.status) === statusFilter;
                  ...
                  <tr key={contact.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex justify-center space-x-2">
                        ...
                        <button onClick={() => handleDelete(contact.id)} className="px-3 py-1.5 bg-red-500 text-white rounded hover:bg-red-600 whitespace-nowrap writing-normal">
                          삭제
                        </button>
                      </div>
                    </td>
            ...
            {paginatedContacts.map((contact, index) => (
              ...
              >
                ...
                <div className="mt-4 flex justify-end space-x-2">
                  ...
                  <button onClick={() => handleDelete(contact.id)} className="px-3 py-1.5 bg-red-500 text-white rounded hover:bg-red-600">
                    삭제
                  </button>
                </div>
              </div>
          ...
          <div className="bg-white rounded-lg p-6 w-full max-w-md">
            ...
            <div className="flex justify-end">
              <button
                ...
                className="w-full px-4 py-2 bg-gray-100 text-green-800 rounded hover:bg-green-200"
              >
                취소
              </button>
            </div>
